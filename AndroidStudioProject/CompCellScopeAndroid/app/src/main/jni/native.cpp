...
JNIEXPORT void JNICALL Java_com_wallerlab_processing_tasks_ComputeDPCRefocusTask_computeDPCRefocus(JNIEnv* env, jobject, jfloat zMin, jfloat zStep,
    ...
    for (float zDist = zMin; zDist <= zMax; zDist += zStep)
    {
        ...
        imgCount = imageStack->size();
        //above comment out please
        ...
        snprintf(bfFilename,sizeof(bfFilename), "/storage/emulated/0/CellScope/Full_Scan_Dataset-Sample/Refocused/BF_%d.jpg",(int) zDist);
        snprintf(dpcLRFilename,sizeof(dpcLRFilename), "/storage/emulated/0/CellScope/Full_Scan_Dataset-Sample/Refocused/DPCLR_%d.jpg",(int) zDist);
        snprintf(dpcTBFilename,sizeof(dpcTBFilename), "/storage/emulated/0/CellScope/Full_Scan_Dataset-Sample/Refocused/DPCTB_%d.jpg",(int) zDist);

        __android_log_print(ANDROID_LOG_INFO, "DPC", "writing images to MEEERP, type: %d", results[0].type());
...
void computeFocusDPC(vector<R_image> iStack, int fileCount, float z, int width, int height, int xcrop, int ycrop, Mat* results) {
    __android_log_print(ANDROID_LOG_INFO, "DPC", "entered computeFocusDPC");
